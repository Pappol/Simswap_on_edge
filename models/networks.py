...
class LocalEnhancer(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=32, n_downsample_global=3, n_blocks_global=9, 
        ...
        for n in range(1, n_local_enhancers+1):
            ...
            ### upsample
            model_upsample += [DWConvTranspose2d(ngf_global * 2, ngf_global, kernel_size=3, stride=2, padding=1, output_padding=1), 
                               norm_layer(ngf_global), nn.ReLU(True)]      
...
class ResnetBlock(nn.Module):
    ...
    def build_conv_block(self, dim, padding_type, norm_layer, activation, use_dropout):
        ...
        conv_block += [DWConv(dim, dim, kernel_size=3, padding=p),
                       norm_layer(dim),
                       activation]
        if use_dropout:
...
class Generator_Adain(nn.Module):
    def __init__(self, input_nc, output_nc, latent_size, ngf=64, n_downsampling=2, n_blocks=4, norm_layer=nn.BatchNorm2d,
        ...
        for i in range(n_downsampling):
            mult = 2 ** (n_downsampling - i)
            Dec += [DWConvTranspose2d(ngf * mult, int(ngf * mult / 2), kernel_size=3, stride=2, padding=1,
                                         output_padding=1),
                      norm_layer(int(ngf * mult / 2)), activation]
...
class NLayerDiscriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, use_sigmoid=False, getIntermFeat=False):
        ...
        kw = 4
        padw = 1
        sequence = [[DWConv(input_nc, ndf, kernel_size=kw, stride=2, padding=padw), nn.LeakyReLU(0.2, True)]]
        ...
        for n in range(1, n_layers):
            ...
            nf = min(nf * 2, 512)
            sequence += [[
                DWConv(nf_prev, nf, kernel_size=kw, stride=2, padding=padw),
        ...
        nf = min(nf * 2, 512)
        sequence += [[
            DWConv(nf_prev, nf, kernel_size=kw, stride=1, padding=padw),
            norm_layer(nf),
            nn.LeakyReLU(0.2, True)
        ]]

        if use_sigmoid:
            sequence += [[DWConv(nf, 1, kernel_size=kw, stride=1, padding=padw), nn.Sigmoid()]]
        else:
            sequence += [[DWConv(nf, 1, kernel_size=kw, stride=1, padding=padw)]]
